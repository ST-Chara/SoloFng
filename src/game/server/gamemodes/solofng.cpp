...
#define TPLAYER(C) (((C) < 0 || (C) >= MAX_CLIENTS) ? 0 : (GS->m_apPlayers[C]->m_IsReadyToPlay && GS->IsClientPlayer(C)) ? GS->m_apPlayers[C] : 0)
...
void CGameControllerSoloFNG::DoInteractions()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (!pChr)
			continue;
		int FrzTicks = pChr->m_FreezeTick;
		...
		if (Col == 9 || Col == 10 || Col == 11)
		{
			...
			if (FrzTicks > 0)
			{
				int ShrineTeam = Col == 10 ? TEAM_RED : Col == 11 ? TEAM_BLUE : -1;
		...
		if (FrzTicks > 0)
		{
			if ((FrzTicks+1) % TS == 0)
			{
				if (pChr->GetPlayer()->GetTeam() != -1)
					GW->CreateDamageInd(pChr->m_Pos, 0, (FrzTicks+1) / TS, CmaskOne(pChr->GetPlayer()->GetCID()));
				else
					GW->CreateDamageInd(pChr->m_Pos, 0, (FrzTicks+1) / TS, m_aCltMask[pChr->GetPlayer()->GetTeam()&1]);
					
				if (g_Config.m_SvTickSound == 1) GW->CreateSound(GS->m_apPlayers[i]->m_ViewPos, SOUND_WEAPON_NOAMMO, CmaskOne(i));
				if (g_Config.m_SvTickSound == 2) GW->CreateSound(pChr->m_Pos, SOUND_WEAPON_NOAMMO);
			}
